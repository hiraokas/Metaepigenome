...
Description:
    ...
    History: 20251223
    - Script for read mapping on reference genome.
Require:
    -t        mapping tools [bowtie2, bwa, bwa_matepair, bowtie2strict
                             pbmm2, pbmm2CCS, pbmm2HiFi, pbmm2CCSHiFi, minimapONT, minimapHiFi]
                             (abort)[blasr, ]
...
Option:
    ...
    -P        Indicate final check filepath.
...
bwa="bwa"
blasr="        ${HOME}/miniconda3/envs/py38/bin/blasr"
pitchfork_env="${HOME}/software/pitchfork/deployment/setup-env.sh"
pbmm2="        ${HOME}/miniconda3/envs/py38/bin/pbmm2"
#pbmm2="        ${HOME}/workspace/software/smrtlink_11.1.0.166339/smrtcmds/bin/pbmm2"
...
while getopts t:T:XYRb:i:h1:2:IP OPT
do
    case $OPT in
        T)  THREADS=$OPTARG
            ;;
        ...
        I)  FLAG_ONLYMAKINGINDEX=true
            ;; 
        I)  FLAG_ONLYOUTPUTCHECKFILEPATH=true
            ;;   
        b)  BASE_FILE=$OPTARG
            FILENAME=${BASE_FILE##*/}
...
if [ ${MAPPING_TOOL} = "bowtie2" ] || [ ${MAPPING_TOOL} = "bowtie2strict" ]  ; then
    ...
    if [ ! -e ${output_bam_check} ]; then
        ...
        if [ ${MAPPING_TOOL} = "bowtie2" ]; then
            command="${bowtie2} -N 1 -x ${INDEX_NAME} ${INPUTFILE_OPTION} ${read_format} --quiet --threads ${THREADS} | ${samtools} view -bS -@ ${THREADS} -o ${output_bam}"
        elif [ ${MAPPING_TOOL} = "bowtie2strict" ]; then
            command="${bowtie2} -N 0 -x ${INDEX_NAME} ${INPUTFILE_OPTION} ${read_format} --quiet --threads ${THREADS} | ${samtools} view -bS -@ ${THREADS} -o ${output_bam}"
        fi
...
#--------------------------------------------------------------------------------------------------
    ...
    if [ ! -e ${output_bam_check} ]; then
        ...
        echo "@@@ Skip Mmpping: ${output_bam} is already exist."
    fi   
