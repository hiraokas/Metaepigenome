...
Description:
    ...
    History:  20200224
    History:  20220221
    History:  20230315 (update mafft7.5)
    History:  20250225 (add mode fasttree_trimAI)
    History:  20251225
...
Usage:
    ./phylogenetic_tree_construction.sh [command] [<options>]
Required:
    -i    input multi-fasta file [.faa(protein)/.fasta(nucle)]
    -S    tools or priset [raxml, fasttree, fasttree16S, fasttreeAmoA, fasttreeTrimal]
              - fasttree       : JTT + CAT
              - fasttree16S    : gtr, cat
              - fasttreeAmoA   : gamma, gtr, cat
              - fasttreeTrimal : trimAI, JTT + CAT
Options:
...
trimal="       ${HOME}/software/trimal-1.5.0/source/trimal"
...
output_dir="${output_dir_prefix}/${QUERY_BASE_FILENAME}_${TREE_TOOL}"
...
if [ ! -e ${output_dir} ]; then
    ...
    echo "Skiped blast search because already files exist. Output dir: ${output_dir}"
fi
...
elif [ ${TREE_TOOL} = "fasttree16S" ]; then
    echo "FastTree START"
...
elif [ ${TREE_TOOL} = "fasttreeAmoA" ]; then
    echo "FastTree START"
...
elif [ ${TREE_TOOL} = "fasttreeTrimal" ]; then
    echo "trimal start befor tree construction"
    output_prefix=`echo ${output_align_file} | sed -e "s/.aln$//g"`
    command="${trimal} -in ${output_align_file} -out ${output_prefix}.trimal -htmlout ${output_prefix}.trimal.html -automated1"
    echo ${command}; eval ${command}

    echo "FastTree START"
    echo "Phylogenetic tree reconstruction using FastTree... (Jones-Taylor-Thorton + CAT model)"  
    export OMP_NUM_THREADS=${THREADS}
    command="${FastTree} -log ${output_tree_file_log} ${output_prefix}.trimal > ${output_tree_file}"
    echo ${command}; eval ${command}
